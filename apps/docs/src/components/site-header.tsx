...
function GitHubMark(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 16 16" aria-hidden="true" width="14" height="14" {...props}>
      <path
        fill="currentColor"
    ...
    <PageRail className="pt-6" ambience={isHome}>
      <header className="relative flex flex-col gap-3">
        <MastheadRow
          ...
          trailing={
            <>
              <nav className="ml-auto flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.8rem] text-muted-foreground">
                {nav.map((item) => (
                  <Link
                    key={item.to}
                    to={item.to}
              ...
              <Button
                variant="ghost"
                size="icon-sm"
                render={<a href="https://github.com/DimitriGilbert/workspace-welcome" />}
                aria-label="GitHub"
              >
