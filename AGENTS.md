...
Tests & db: `pnpm --filter @workspace-welcome/api test:store` / `test:forge` / `test:clone` (node:test suites over temp XDG dirs); `pnpm --filter @workspace-welcome/db db:selftest`, and `db:generate` (dev-time drizzle-kit diff base for the embedded migrations)
