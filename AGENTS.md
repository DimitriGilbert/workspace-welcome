...
Prod server: the built dashboard runs as the systemd user unit `workspace-welcome.service` (port 37420, serves `apps/web/dist`, logs to the journal). After a rebuild, refresh it with `systemctl --user restart workspace-welcome` — never kill/spawn `serve-prod.mjs` by hand; a rogue process on the port trips the unit's start limit and leaves the service dead.
Validation: exactly two sanctioned ways to reach the app — the dev server (`pnpm dev`; assume it is already running) or the prod service above. Point widget-check at whichever holds port 37420 (`--base-url`). Never start private prod servers on spare ports, never `pkill`/kill by pattern — process management is `systemctl` for the service and nothing else.
Tests & db: `pnpm --filter @workspace-welcome/api test:store` / `test:forge` / `test:clone` (node:test suites over temp XDG dirs); `pnpm --filter web test:board` (widget-layout persistence unit tests); `pnpm --filter @workspace-welcome/db db:selftest`, and `db:generate` (dev-time drizzle-kit diff base for the embedded migrations)
